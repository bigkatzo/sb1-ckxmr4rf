-- Fix get_products_by_launch_date to use the same data source as get_trending_products
BEGIN;

-- Drop the existing function
DROP FUNCTION IF EXISTS public.get_products_by_launch_date(integer, integer);

-- Recreate the function using public_trending_products view for consistency
...
AS $$
  WITH ranked_products AS (
    SELECT 
      p.id,
      tp.all_time_orders AS order_count, -- Use all_time_orders from trending_products
      ROW_NUMBER() OVER (
        ORDER BY p.collection_launch_date DESC, p.id -- Still sort by launch date
      ) AS rank
