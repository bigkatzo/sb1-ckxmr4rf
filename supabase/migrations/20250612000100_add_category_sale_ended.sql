-- Add sale_ended column to categories table
ALTER TABLE categories
ADD COLUMN IF NOT EXISTS sale_ended boolean NOT NULL DEFAULT false;

-- Create index for faster queries
CREATE INDEX IF NOT EXISTS idx_categories_sale_ended
ON categories(sale_ended)
WHERE sale_ended = true;

-- Create function to toggle category sale ended status
CREATE OR REPLACE FUNCTION toggle_category_sale_ended(
  p_category_id uuid,
...
-- Update merchant_products view to include category_sale_ended
...
-- Update public_products view to include category_sale_ended
