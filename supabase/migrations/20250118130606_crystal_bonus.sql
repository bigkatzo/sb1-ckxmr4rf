...
BEGIN
  ...
  EXCEPTION
    WHEN others THEN
      ...
      RAISE WARNING 'Failed to create profile for user %: %', v_user_id, SQLERRM;
  END;
...
BEGIN
  ...
  v_role := CASE 
    ...
    WHEN NEW.raw_app_meta_data->>'role' IN ('admin', 'merchant', 'user') THEN NEW.raw_app_meta_data->>'role'
    ELSE 'user'
  END;
...
-- Create function to ensure profile exists with correct role
...
BEGIN
  ...
  -- Create or update profile with correct role
  INSERT INTO user_profiles (id, role)
...
-- Create trigger for ensuring profile exists
...
SET role = CASE 
  ...
  WHEN u.raw_app_meta_data->>'role' IN ('admin', 'merchant', 'user') THEN u.raw_app_meta_data->>'role'
  ELSE 'user'
END
FROM auth.users u
WHERE p.id = u.id;
