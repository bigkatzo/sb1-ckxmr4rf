...
BEGIN
  ...
  IF (
    ...
    url ~* '^https?://[a-z0-9-]+\.supabase\.co/' OR
    url ~* '^https?://[a-z0-9-]+\.cloudinary\.com/'
  ) THEN
    -- Check file extension or content type hint in URL
    IF (
...
BEGIN
  ...
  IF NEW.image_url IS NOT NULL THEN
    IF NOT validate_image_url(NEW.image_url) THEN
      ...
      NEW.image_url := NULL;
      RAISE WARNING 'Invalid collection image URL, setting to null';
    END IF;
  END IF;
...
BEGIN
  ...
  -- Filter out invalid images instead of raising error
  NEW.images := (
  ...
  IF array_length(NEW.images, 1) IS NULL THEN
    RAISE WARNING 'No valid images found for product';
  END IF;
