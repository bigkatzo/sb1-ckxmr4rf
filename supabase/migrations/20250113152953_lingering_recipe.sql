...
VALUES 
  ...
  ('product_images', 'product_images', true, 5242880, ARRAY['image/jpeg', 'image/png', 'image/webp', 'image/gif'])
...
CREATE POLICY "Storage read"
  ...
  TO public
  USING (bucket_id IN ('collection-images', 'product_images'));
...
CREATE POLICY "Storage write"
  ...
  TO authenticated
  WITH CHECK (
    bucket_id IN ('collection-images', 'product_images')
