-- Add comprehensive access management system
BEGIN;

-- Function to get current collection access for management UI
...
BEGIN
  ...
  SELECT COALESCE(json_agg(
    json_build_object(
      ...
      'merchant_tier', COALESCE(up.merchant_tier, 'starter_merchant'),
...
-- Function to manage collection access (add, update, remove, transfer)
...
BEGIN
  ...
  CASE p_action
    ...
    WHEN 'transfer_ownership' THEN
      -- Transfer ownership (same logic as before but integrated)
