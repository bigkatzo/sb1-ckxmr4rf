...
BEGIN
  ...
  -- Generate numeric timestamp (YYYYMMDDHHMMSS)
  timestamp := regexp_replace(to_char(now(), 'YYYYMMDDHHMMSS'), '[^0-9]', '', 'g');
  
  -- Generate alphanumeric random string
  random_str := regexp_replace(substr(encode(gen_random_bytes(4), 'hex'), 1, 8), '[^a-z0-9]', '', 'g');

  -- Clean filename:
  -- 1. Remove extension
  -- 2. Convert to lowercase
  -- 3. Remove ALL non-alphanumeric chars
  sanitized := regexp_replace(
    lower(regexp_replace(filename, '\.' || extension || '$', '')),
    '[^a-z0-9]', '', 'g'
  );

  -- Add timestamp and random string (all alphanumeric)
