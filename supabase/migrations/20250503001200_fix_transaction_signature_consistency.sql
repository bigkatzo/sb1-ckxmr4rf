...
-- Rename transaction_id to transaction_signature in orders table if it exists
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 
    ...
    ALTER TABLE orders RENAME COLUMN transaction_id TO transaction_signature;
  END IF;
END $$;
...
-- Update recover_failed_order function to use transaction_signature and remove transaction_status
