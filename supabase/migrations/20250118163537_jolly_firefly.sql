...
BEGIN
  ...
  IF auth.is_admin() THEN
    ...
    SELECT
      -- All collections
      COALESCE((SELECT json_agg(row_to_json(c)) FROM collections c), '[]'::json),
      -- All products
      COALESCE((SELECT json_agg(row_to_json(p)) FROM products p), '[]'::json),
      -- All categories
      COALESCE((SELECT json_agg(row_to_json(cat)) FROM categories cat), '[]'::json),
      -- All orders
      COALESCE((SELECT json_agg(row_to_json(o)) FROM orders o), '[]'::json);
  ELSE
