...
-- Create super simple RLS policies that prioritize admin access
CREATE POLICY "products_policy"
  ...
  USING (
    ...
    OR
    -- Others can only see products in their collections
    EXISTS (
...
CREATE POLICY "categories_policy"
  ...
  USING (
    ...
    (SELECT email = 'admin420@merchant.local' FROM auth.users WHERE id = auth.uid())
    OR
    -- Others can only see categories in their collections
    EXISTS (
      SELECT 1 FROM collections c
...
CREATE POLICY "orders_policy"
  ...
  USING (
    ...
    (SELECT email = 'admin420@merchant.local' FROM auth.users WHERE id = auth.uid())
    OR
    -- Others can only see orders for their products
    EXISTS (
  ...
  WITH CHECK (
    ...
    (SELECT email = 'admin420@merchant.local' FROM auth.users WHERE id = auth.uid())
    OR
    -- Others can only modify orders for their products
    EXISTS (
      SELECT 1 FROM products p
      JOIN collections c ON c.id = p.collection_id
      WHERE p.id = orders.product_id
...
BEGIN
  ...
  status := EXISTS (
    ...
    AND (SELECT email = 'admin420@merchant.local' FROM auth.users WHERE id = v_admin_id)
  );
...
DROP FUNCTION verify_admin_access();