...
BEGIN
  ...
  -- Generate timestamp without special chars
  timestamp := to_char(now(), 'YYYYMMDDHHMMSS');
  
  -- Generate random string without special chars
  random_str := substr(encode(gen_random_bytes(4), 'hex'), 1, 8);
  ...
  -- Add timestamp and random string without special chars
