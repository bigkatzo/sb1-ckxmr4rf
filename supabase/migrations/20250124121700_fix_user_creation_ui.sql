...
BEGIN
  ...
  VALUES (
    ...
    jsonb_build_object(                      -- raw_app_meta_data
      'provider', 'email',                   -- changed provider to 'email'
      'providers', ARRAY['email']            -- set providers array to ['email']
    ),
  ...
  -- Fix any existing users that might have incorrect fields
  UPDATE auth.users
  SET 
    aud = 'authenticated',
...
-- Add a trigger to ensure these fields are always set correctly
