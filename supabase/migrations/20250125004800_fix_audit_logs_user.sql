...
-- Update the audit logging function to handle missing user
...
BEGIN
    ...
    -- Try to get the authenticated user, fall back to the collection owner
    BEGIN
        acting_user := auth.uid();
    EXCEPTION WHEN OTHERS THEN
        acting_user := NEW.collection_owner_id;
        IF acting_user IS NULL THEN
            acting_user := OLD.collection_owner_id;
        END IF;
    END;

    -- If we still don't have a user, use a system user ID
    IF acting_user IS NULL THEN
        -- Use a fixed UUID for system actions
        acting_user := '00000000-0000-0000-0000-000000000000'::uuid;
    END IF;
...
-- Make sure system user exists in audit context
DO $$
BEGIN
    INSERT INTO auth.users (id, email)
    VALUES ('00000000-0000-0000-0000-000000000000', 'system@internal')
