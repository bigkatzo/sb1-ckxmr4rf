...
export function CollectionLinks({ collection, className = '' }: CollectionLinksProps) {
  ...
  // DOM manipulation to manage the expanded content outside the hero card
  useEffect(() => {
    ...
    if (isExpanded && detailsContainer) {
      ...
      // Render the content (we need to do this manually since we're moving it outside React's control)
      ...
      if (contentPlaceholder && contentPlaceholder instanceof HTMLElement) {
        // Use React's createPortal to render the actual content
  ...
  // We're only rendering the toggle button here, content is rendered via DOM manipulation
  return (
