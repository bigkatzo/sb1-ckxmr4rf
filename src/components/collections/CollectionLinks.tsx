...
export function CollectionLinks({ collection, className = '' }: CollectionLinksProps) {
  ...
  const [mobileContent, setMobileContent] = useState<React.ReactNode>(null);
  ...
  // Update the useEffect for mobile content
  useEffect(() => {
            ...
            <div className="mb-3">
              ...
              {isFetchingProfile ? (
                ...
                <div className="inline-flex items-center gap-1.5 bg-white/10 text-white px-2 py-1 rounded-full">
                  <div className="h-5 w-5 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center text-white/70 bg-white/20 text-[10px]">A</div>
    ...
    const handleResize = () => {
      ...
      if (!isMobileNow) {
        ...
        const content = document.querySelector('.collection-expanded-content');
        if (content) {
          content.remove();
        }
      }
