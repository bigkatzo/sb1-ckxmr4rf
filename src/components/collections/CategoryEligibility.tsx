      ...
      <div className="text-[10px] sm:text-xs text-text-muted">
        ...
        <div className="space-y-2 ml-2">
          <div className="flex items-start">
            ...
            <span className="text-text">This category is open to all collectors</span>
          </div>
        </div>
      </div>
    );
      ...
      {groups.map((group, groupIndex) => (
        <div key={groupIndex} className="space-y-2">
                ...
                {group.rules.map((rule, ruleIndex) => (
                  <div key={ruleIndex} className="flex items-start">
                    ...
                    {ruleIndex < group.rules.length - 1 && (
                      <span className="mx-2 text-text-secondary font-medium">
                        {group.operator}
                      </span>
                    )}
                  </div>
          ...
          {groupIndex < groups.length - 1 && group.rules.length > 0 && (
            <div className="text-[10px] sm:text-xs text-text-secondary font-medium text-center">
              AND
            </div>
          )}
        </div>
