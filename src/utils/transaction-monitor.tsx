  ...
  try {
    ...
    while (attempts < MAX_RETRIES) {
      try {
        ...
        if(status?.err) {
          console.error(`[TRANSACTION_MONITOR] Transaction error detected:`, status.err);
          const errorMessage = status.err ? JSON.stringify(status.err) : 'Transaction failed with an unknown error';
